...
class Go23SendViewController: UIViewController {
    ...
    private func supportClick(selected: Bool) {
        ...
        if selected {
            attri.addBundleImage("blueSel", CGRectMake(0, -2, 16, 16))
        } else {
            attri.addBundleImage("graySel", CGRectMake(0, -2, 16, 16))
        }
    ...
    private lazy var scanBtn: UIButton = {
        ...
        let imgv = UIImageView()
        imgv.image = Go23Helper.image(named: "grayScan")
        btn.addSubview(imgv)
        imgv.snp.makeConstraints { make in
            make.center.equalToSuperview()
    ...
    private lazy var clearBtn: UIButton = {
        let btn = UIButton(type: .custom)
        let imgv = UIImageView()
        imgv.image = Go23Helper.image(named: "clear")
        btn.addSubview(imgv)
        imgv.snp.makeConstraints { make in
            make.center.equalToSuperview()
...
class SendHeaderView: UIView {
    ...
    private lazy var arrowImgv: UIImageView = {
        let imgv = UIImageView()
        imgv.image = Go23Helper.image(named: "blueNext")
        return imgv
    }()
